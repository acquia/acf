...
class addToCartForm extends HTMLElement {
 ...
 buildButton() {
  ...
  async buttonClick() {
   ...
   let qty = this.querySelector('.coh-style-quantity select').value;
   let attr = this.attr;
  ...
  buildQuantitySelect() {
   ...
   let acfselect = document.createElement('acf-select');
   acfselect.classList.add('coh-style-quantity');
   acfselect.setAttribute('name', 'quantity');
   // Add the label & select to the wrapper.
   wrapper.append(label);
   wrapper.append(acfselect);
